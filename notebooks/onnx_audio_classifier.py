...
def kfold_cross_validation(
    data_path, species, model_name, k_folds=5, batch_size=32, epochs=10, lr=0.001
):
    """
    ...
    files = glob.glob(
        os.path.join(data_path, species, data_subfolder_name, "**/*.wav"),
        recursive=True,
    )
    labels_df = pd.DataFrame(
        {"file": files, "present": ["pos" in f.lower() for f in files]}
    )
    ...
    for train_idx, val_idx in kf.split(X):
        ...
        try:
            ...
            if len(np.unique(val_labels)) > 1:
                ...
                print(
                    f"Fold {fold_idx} - Warning: only one class in validation set, skipping AUC calculation"
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train ONNX models for audio classification"
    )
    parser.add_argument(
        "--data_path",
        type=str,
    ...
    parser.add_argument(
        "--species", type=str, default="bullfrog", help="Species to process"
    )
    parser.add_argument(
        "--model",
        type=str,
