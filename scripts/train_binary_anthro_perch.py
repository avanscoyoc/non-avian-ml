"""
train_binary_anthro_perch.py

Binary Perch classifier: anthropogenic (pos=1) vs natural (neg=0).
- Uses ALL available .wav files from each species' data_5s/pos/ directory.
- Straight 80/20 train/test split, no k-fold, no per-class subsampling.
- CrossEntropyLoss with inverse-frequency class weights.

Usage:
    python scripts/train_binary_anthro_perch.py
"""
...
def extract_embeddings(model: PerchModel, files: list[str]) -> torch.Tensor:
    embeddings = []
...
def main():
    ...
    train_embs = extract_embeddings(perch, train_files)

    print(f"\nExtracting test embeddings ({len(test_files)} files)...")
    test_embs  = extract_embeddings(perch, test_files)
    ...
    plot_confusion_matrix(cm, figs_path / "confusion_matrix_binary_anthro_perch.png")
    ...
    save_classifier_bundle(
        ...
        bundle_name="binary_anthro_perch",
        labels={
            "0": "natural",
            "1": "anthropogenic",
